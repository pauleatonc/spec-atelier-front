...
const NavBarLogin = ({ fixed }) => {
  ...
  const imgLogo = () => {
    ...
    if (window.matchMedia(MAX_SCREEN_STANDAR_MEDIUM).matches)
      ...
      src = transparentize ? LOGO : LOGO_MOBILE;

    return <ImgLogo src={src} transparent={transparentize} alt='Logotipo de SpecAtelier' />
  }
  ...
  return (
    <NavbarContainer transparent={transparentize} fixed={fixed}>
      <NavbarContent>
        ...
        <ItemsContainer>
          {items.map((item) => (
            <Link key={item.id} to={item.to} data-view={item.id} style={{ textDecoration: 'none' }} >
              <Item active={item.id === currentLocation} transparent={transparentize}>
                <ItemText>{item.label}</ItemText>
              </Item>
            </Link>
          ))}
          {isLogin ? <NavProfile /> : <NavBarLoginRegister transparent={transparentize} />}
        </ItemsContainer>
      </NavbarContent>
      <LoginRegisterBurgerMenuContent>

...
export const onRemoveSpecBlock = ({ blockID, specID }) => async (dispatch, getState) => {
  dispatch(onActionCreator(REMOVE_SPEC_BLOCK));

  try {
    const { auth } = getState();
    const { blocks: updatedBlocks } = await deleteSpecBlock({ blockID, specID, userID: auth.user?.id });

    return dispatch(onActionCreator(REMOVE_SPEC_BLOCK_SUCCESS, { blocks: updatedBlocks }));
  } catch (error) {
...
export const onDetachSpecProduct = ({ productID, specID }) => (dispatch, getState) => {
  const { specDocument } = getState();
  const selectedBlock = specDocument.blocks.find(block => block.element?.id === productID) || {};
...
export const onAddSpecBlockImage = ({ blockID, imageID, specID }) => async (dispatch, getState) => {
  dispatch(onActionCreator(ADD_SPEC_BLOCK_IMAGE));

  try {
    const { auth } = getState();
    const { blocks: updatedBlocks } = await updateSpecBlockImage({ blockID, imageID, specID, userID: auth.user?.id });
...
export const REMOVE_SPEC_BLOCK_IMAGE_SUCCESS = 'REMOVE_SPEC_BLOCK_IMAGE_SUCCESS';
export const onRemoveSpecBlockImage = ({ blockID, specID }) => async (dispatch, getState) => {
  dispatch(onActionCreator(REMOVE_SPEC_BLOCK_IMAGE));

  try {
    const { auth } = getState();
    const { blocks: updatedBlocks } = await deleteSpecBlockImage({ blockID, specID, userID: auth.user?.id });

    return dispatch(onActionCreator(REMOVE_SPEC_BLOCK_IMAGE_SUCCESS, { blocks: updatedBlocks }));
  } catch (error) {
    return dispatch(onActionCreator(REMOVE_SPEC_BLOCK_IMAGE_ERROR, { error: true, nativeError: error }));
  }
};
...
export const onAddSpecBlockText = ({ blockID, specID, textValue }) => async (dispatch, getState) => {
  ...
  try {
    ...
    return dispatch(onActionCreator(ADD_SPEC_BLOCK_TEXT_SUCCESS, { blocks: updatedBlocks }));
  } catch (error) {
...
export const onRemoveSpecBlockText = ({ textID, specID }) => async (dispatch, getState) => {
  dispatch(onActionCreator(REMOVE_SPEC_BLOCK_TEXT));

  try {
    const { auth } = getState();
    const { blocks: updatedBlocks } = await deleteSpecBlockText({ specID, textID, userID: auth.user?.id });

    return dispatch(onActionCreator(REMOVE_SPEC_BLOCK_TEXT_SUCCESS, { blocks: updatedBlocks }));
  } catch (error) {
    return dispatch(onActionCreator(REMOVE_SPEC_BLOCK_TEXT_ERROR, { error: true, nativeError: error }));
  }
};
...
export const onUpdateSpecBlockText = ({ blockID, specID, textID, textValue }) => async (dispatch, getState) => {
  ...
  try {
    ...
    return dispatch(onActionCreator(UPDATE_SPEC_BLOCK_TEXT_SUCCESS, { blocks: updatedBlocks }));
  } catch (error) {
    return dispatch(onActionCreator(UPDATE_SPEC_BLOCK_TEXT_ERROR, { error: true, nativeError: error }));
  }
};
...
export const onSortSpecBlocks = ({ blocksIDs, specID }) => async (dispatch, getState) => {

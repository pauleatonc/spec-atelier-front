...
const SpecContents = () => {
	...
	const handleSectionClick = sectionID => () => {
		...
		setSelectedSection((currentSelectedSection) => {
			if (currentSelectedSection === sectionID) { return undefined };
			return sectionID;
		});
	};
	...
	const handleItemClick = itemID => () => {
		setSelectedItem(currentSelectedItem => {
			if (currentSelectedItem === itemID) { return undefined };
			return itemID;
		});
	};
	...
	return (
		<>
			...
			<Root show={show}>
				<PanelTitle>
					<ButtonBack role='button' onClick={() => dispatch(onHideSpecContents())}>
				...
				{sections.map(section => (
					<Fragment key={section.id}>
						...
						>
							...
							<ArrowIcon
								src={selectedSection === section.id ? ARROW_UP_SOURCE : ARROW_DOWN_SOURCE}
							/>
						</ListItem>
							...
							{section.items.map(item => (
								<Fragment key={item.id}>
									...
									>
										...
										<ArrowIcon
											src={selectedItem === item.id ? ARROW_UP_SOURCE : ARROW_DOWN_SOURCE}
										/>
									</ListItem>

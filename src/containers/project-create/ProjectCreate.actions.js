...
import { addProjectToList, GET_PROJECTS } from '../projects-list/ProjectsList.actions';
...
export const getProject = ({ id }) => async (dispatch, getState) => {
  try {
    const { user } = getState().auth;
    const response = await getProjectService({ userId: user.id, projectId: id });
    if (response?.status >= 400) return dispatch(onActionCreator(GET_PROJECT_ERROR, { loading: false, error: response?.error }));
    const {
      cities = [],
      project_types = [],
      work_types = [],
    ...
    const project = {
      ...
      project_type: project_types.find(pt => pt.value === response.project?.project_type) || {},
      work_type: work_types.find(wt => wt.value === response.project?.work_type) || {},
      city: cities.find(c=> c.name === response.project.city) || {},
    }
    dispatch(onActionCreator(GET_PROJECT, { project, loading: false }));
  } catch (error) {
    console.log('reaerrorsdasdasd', error)

    return dispatch(onActionCreator(GET_PROJECT_ERROR, { loading: false, error }));
  }
};
...
export const modifyProject = project => async (dispatch, getState) => {
  try {
    const { user } = getState().auth;
    const response = await modifyProjectService({ userId: user.id, projectId: project.id, data: project });
    if (response?.status >= 400) return dispatch(onActionCreator(CREATE_PROJECT_ERROR, { loading: false }));

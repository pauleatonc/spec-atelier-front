...
export const getClient = clientId => async dispatch => {
  try {
    ...
    if (response?.status >= 400) return dispatch(onActionCreator(GET_CLIENT_ERROR, { loading: false, error }));
    return dispatch(onActionCreator(GET_CLIENT, { client: response.client, loading: false }));
  } catch (error) {
    return dispatch(onActionCreator(GET_CLIENT_ERROR, { loading: false, error }));
  }
};

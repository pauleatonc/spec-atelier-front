...
import { getMySpecs, getRoomTypes as getRoomTypesService } from '../../services/specs.service';
...
export const onGetSpecProducts = ({ key = '', value } = {}) => async (dispatch, getState) => {
  dispatch(onActionCreator(GET_SPEC_PRODUCTS));

  try {
    const { app, brandsList, specProducts } = getState();
    const filters = Object.entries({ ...specProducts.filters, [key]: value }).reduce((entries, [key, value]) => {
    ...
    const response = await getProducts(cleanObjectsAndArrays(filters), GET_SPEC_PRODUCTS);
...
export const UPDATE_SPEC_PROJECT_TYPE_FILTERS = 'UPDATE_SPEC_PROJECT_TYPE_FILTERS';
export const onGetSpecProductsProjectType = ({ project_types }) => async dispatch => {
  // try {
    // const { room_types = [] } = await getRoomTypesService({ params: { project_types }});
    batch(() => {
      dispatch(getRoomTypes({ project_types }));
      dispatch(onActionCreator(UPDATE_SPEC_PROJECT_TYPE_FILTERS, { room_types, project_types }));
...
export const getMySpecifications = () => async dispatch => {
  try {
    const { specifications } = await getMySpecs({ params: { with_products: true } });

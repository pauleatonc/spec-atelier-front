...
export const getMyProjects = (params) => async (dispatch, getState) => {
  try {
    ...
    dispatch(
      onActionCreator(GET_PROJECTS, { projects, loading: false, params }),
    );
  } catch (error) {
    dispatch(
      onActionCreator(GET_PROJECTS_ERROR, {
        loading: false,
        error: true,
        params,
      }),
...
export const getMoreProjects = (params) => async (dispatch, getState) => {
  try {
    ...
    const { projects } = await getProjects({ userId: user.id, params });
    dispatch(
      onActionCreator(GET_MORE_PROJECTS, { projects, loading: false, params }),
    );
  } catch (error) {
    dispatch(
      onActionCreator(GET_PROJECTS_ERROR, {
...
export const deleteProject = ({ id }) => async (dispatch) => {
  try {
    ...
    dispatch(
      onActionCreator(DELETE_PROJECT_ERROR, { loading: false, error: true }),
    );
  }
};
  ...
  acceptNotification(body).then(
    (response) => {
      ...
      if (response.codeStatus === 200) {
        dataResp.then((data) => {
          dispatch(onShowAlertSuccess({ message: data.message }));
        });
        dispatch(onActionCreator(ACCEPT_NOTIFICATION, response));
      }
    },
    (error) => {
      dispatch(onShowAlertSuccess({ message: 'Error al aceptar proyecto.' }));
      dispatch(
        onActionCreator(ACCEPT_NOTIFICATION_ERROR, {
          error,
        }),
      );
    },
...
export const rejectNotificationsAC = (body) => async (dispatch) => {
  rejectNotification(body).then(
    ...
    (error) => {
      dispatch(onShowAlertSuccess({ message: 'Error al rechazar proyecto.' }));
      dispatch(
        onActionCreator(REJECT_NOTIFICATION_ERROR, {
          error,
        }),
      );
    },
